...
void Blimp::failsafe_ekf_event()
{
    ...
    if (!blimp.flightmode->requires_GPS() && (g.fs_ekf_action != FS_EKF_ACTION_BRAKE_EVEN_MANUAL)) {
        return;
    }

    // take action based on fs_ekf_action parameter
    switch (g.fs_ekf_action) {
    case FS_EKF_ACTION_BRAKE:
    case FS_EKF_ACTION_BRAKE_EVEN_MANUAL:
    default:
        set_mode_brake_failsafe(ModeReason::EKF_FAILSAFE);
        break;
    }
}
